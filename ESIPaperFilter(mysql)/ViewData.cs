...
namespace ESIPaperFilter
{
    public partial class ViewData : Form
    {
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            DataTable table = SqlHelper.ExecuteDataSet(CommandType.Text, sql).Tables0];
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex == -1)
                return;
            string id = dataGridView1.Rowse.RowIndex].Cells0].Value.ToString();
            string columnname = dataGridView1.Columnse.ColumnIndex].DataPropertyName;
            string sql = string.Format("update MasteJournalList] set {0}='{1}' where id]={2}", columnname, dataGridView1.Rowse.RowIndex].Cellse.ColumnIndex].Value.ToString(), id);
