...
namespace ESIPaperFilter
{
    public partial class ImportData : Form
    {
        ...
        private void lblmessage_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (items.Length > 0)
                {
                    ...
                    {
                        table.Columns.Add(new DataColumn(headsindex], typeof(string)));
                    }
                    if (items.Length > 1)
                    {
                        for (int index = 1; index < items.Length; index++)
                        {
                            ...
                            for (int colindex = 0; colindex < itemsindex].Length; colindex++)
                            {
                                if (itemsindex]colindex].ToLower() != "null")
                                {
                                    string item = itemsindex]colindex];

                                    if ((items0]colindex].ToLower().Contains("issn") || items0]colindex].ToLower().Contains("eissn")) && !IsISSN(item.Trim()))
                                    {
                                        // item = item.Insert(4, "-");
                                        item = "_" + item;
                                    }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                headdataGridView1.Columnsindex].Name.ToLower()] = index;
            }
            for (int index = 0; index < dataGridView1.Rows.Count; index++)
            {
                Journal journal = new Journal();
                journal.fulltitle = dataGridView1.Rowsindex].Cellshead"full title"]].Value as string;
                journal.title29 = dataGridView1.Rowsindex].Cellshead"title29"]].Value as string;
                journal.issn = dataGridView1.Rowsindex].Cellshead"issn"]].Value as string;
                journal.eissn = dataGridView1.Rowsindex].Cellshead"eissn"]].Value as string;
                journal.title20 = dataGridView1.Rowsindex].Cellshead"title20"]].Value as string;
                journal.category = dataGridView1.Rowsindex].Cellshead"category name"]].Value as string;
                journal.year = year;
                journal.month = month;
